...
def make_out_ncfile():
    ...
    plensemble, plsystem = (
        split_original_path[-2],
        split_original_path[-1],
    )  # plensemble is typically "tyk2_..."; plsystem is typically "out_{lig0}_{lig1}"
...
def load_from_path(_path, _phase, _endstate):
    factory = np.load(_path + "/out-hybrid_factory.npy.npz", allow_pickle=True)[
        "arr_0"
    ].item()[_phase]
...
topology, positions, system = load_from_path(
    args.factory_path, args.phase, args.endstate
)
md_top = md.Topology.from_openmm(topology)
pt_filename = outnc[:-2] + "pt"
mixed_system = repex.MixedSystemConstructor(
    system=system, topology=topology, filename=pt_filename
).mixed_system
