...
class SAMSSampler(multistate.MultiStateSampler):
    ...
    _TITLE_TEMPLATE = (
        "Self-adjusted mixture sampling (SAMS) simulation using SAMSSampler "
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(SAMSSampler, self).__init__(
    ...
    class _StoredProperty(multistate.MultiStateSampler._StoredProperty):
        ...
        def _state_update_scheme_validator(instance, scheme):
            ...
            if scheme not in supported_schemes:
                raise ValueError(
                    "Unknown update scheme '{}'. Supported values "
    ...
    log_target_probabilities = _StoredProperty(
        "log_target_probabilities", validate_function=None
    )
    state_update_scheme = _StoredProperty(
        "state_update_scheme",
        validate_function=_StoredProperty._state_update_scheme_validator,
    )
    locality = _StoredProperty("locality", validate_function=None)
    update_stages = _StoredProperty(
        "update_stages", validate_function=_StoredProperty._update_stages_validator
    )
    flatness_criteria = _StoredProperty(
        "flatness_criteria",
        validate_function=_StoredProperty._flatness_criteria_validator,
    )
    flatness_threshold = _StoredProperty("flatness_threshold", validate_function=None)
    weight_update_method = _StoredProperty(
        "weight_update_method",
        validate_function=_StoredProperty._weight_update_method_validator,
    )
    adapt_target_probabilities = _StoredProperty(
        "adapt_target_probabilities",
        validate_function=_StoredProperty._adapt_target_probabilities_validator,
    )
    ...
    def _pre_write_create(
        self, thermodynamic_states: list, sampler_states: list, storage, **kwargs
    ):
        ...
        if self.log_target_probabilities is None:
            self.log_target_probabilities = np.zeros(
        ...
        if self.logZ_guess is not None:
            if len(self.logZ_guess) != self.n_states:
                raise Exception(
                    "Initial logZ_guess (dim {}) must have same number of states as n_states ({})".format(
    ...
    def _restore_sampler_from_reporter(self, reporter, **kwargs):
        ...
        logger.debug(
            "Restored state histogram: {}".format(self._cached_state_histogram)
        )
        data = reporter.read_online_analysis_data(
            self._iteration, "logZ", "stage", "t0"
        )
    ...
    def _report_iteration_items(self):
        ...
        self._reporter.write_online_data_dynamic_and_static(
            self._iteration, logZ=self._logZ, stage=self._stage, t0=self._t0
        )
        ...
        states, counts = np.unique(
            self._replica_thermodynamic_states, return_counts=True
        )
    ...
    def _mix_replicas(self):
        ...
        logger.debug(
            "Updating thermodynamic states using %s scheme..."
            % self.state_update_scheme
        )
    ...
    def _local_jump(self, replicas_log_P_k):
        ...
        for replica_index, current_state_index in enumerate(
            ...
            proposed_state_index = np.random.choice(
                neighborhood,
                p=np.ones([neighborhood_size], np.float64) / float(neighborhood_size),
            )
            ...
            log_Gamma_j_L = -float(
                proposed_neighborhood_size
            )  # log probability of proposing return
            log_Gamma_L_j = -float(
                neighborhood_size
            )  # log probability of proposing new state
            ...
            for j in neighborhood:
                if j != L:
                    log_P_k[j] = log_Gamma_L_j + min(
                        0.0,
                        log_Gamma_j_L
                        - log_Gamma_L_j
                        + (self.log_weights[j] - u_k[j])
                        - (self.log_weights[L] - u_k[L]),
                    )
    ...
    def _restricted_range_jump(self, replicas_log_P_k):
        ...
        for replica_index, current_state_index in enumerate(
            ...
            logger.debug(
                "  Relative     u_k    : %s"
                % str(u_k[neighborhood] - u_k[current_state_index])
            )
            ...
            proposed_neighborhood = self._neighborhood(proposed_state_index)
            logger.debug(
                "  Proposed neighborhood        : %s" % str(proposed_neighborhood)
            )
            # Accept or reject.
            log_P_accept = logsumexp(
                self.log_weights[neighborhood] - u_k[neighborhood]
            ) - logsumexp(
                self.log_weights[proposed_neighborhood] - u_k[proposed_neighborhood]
            )
            logger.debug("  log_P_accept        : %f" % log_P_accept)
            logger.debug(
                "     logsumexp(g[forward] - u[forward]) : %f"
                % logsumexp(self.log_weights[neighborhood] - u_k[neighborhood])
            )
            logger.debug(
                "     logsumexp(g[reverse] - u[reverse]) : %f"
                % logsumexp(
                    self.log_weights[proposed_neighborhood] - u_k[proposed_neighborhood]
                )
            )
    ...
    def _compute_state_histogram(self, reporter=None):
        ...
        logger.debug(
            "Read replica thermodynamic states: {}".format(replica_thermodynamic_states)
        )
        n_k, _ = np.histogram(
            replica_thermodynamic_states, bins=np.arange(-0.5, self.n_states + 0.5)
        )
        return n_k
    ...
    def _update_stage(self):
        ...
        logger.debug(
            "    state histogram counts ({} total): {}".format(
                self._cached_state_histogram.sum(), self._cached_state_histogram
            )
        )
        if (self.update_stages == "two-stage") and (self._stage == 0):
            ...
            if self.flatness_criteria == "minimum-visits":
                ...
                logger.debug(
                    "logZ-flatness criteria met (%d total): %s"
                    % (np.sum(criteria), str(np.array(criteria, "i1")))
                )
                if np.all(criteria):
                    advance = True
            else:
    ...
    def _update_logZ_estimates(self, replicas_log_P_k):
        ...
        self._reporter.write_online_analysis_data(
            self._iteration, log_weights=self.log_weights
        )
        ...
        for replica_index, state_index in enumerate(self._replica_thermodynamic_states):
            ...
            if self._stage == 0:  # initial stage
                ...
                gamma = self.gamma0 * min(
                    pi_star, (t - self._t0 + self._t0**beta_factor) ** (-1)
                )  # Eq. 15 of [1]
            else:
            ...
            if self.weight_update_method == "optimal":
                ...
                neighborhood = np.where(self._neighborhoods[replica_index, :])[
                    0
                ]  # compact list of states defining neighborhood
