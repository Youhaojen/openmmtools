...
def check_interacting_energy_components(
    ...
    if multi_regions:
        ...
        print(
            "Dissecting reference system's nonbonded force for region {}".format(
                alchemical_regions.name
            )
        )
    else:
    ...
    if multi_regions:
        print(
            "Dissecting alchemical system's unmodified nonbonded force for region {}".format(
                alchemical_regions.name
            )
        )
    else:
    ...
    if multi_regions:
        print(
            "Computing alchemical system components energies for region {}".format(
                alchemical_regions.name
            )
        )
    else:
...
def check_noninteracting_energy_components(
    ...
    for force_name in ["CustomGBForce", "GBSAOBCForce"]:
        ...
        assert_almost_equal(
            reference_force_energy,
            alchemical_energy,
            "reference {}, alchemical {}".format(
                reference_force_energy, alchemical_energy
            ),
        )
...
def check_split_force_groups(system, region_names=None):
    ...
    for region in region_names:
        ...
        if is_alchemical_pme_treatment_exact(system):
            ...
            assert force_groups_by_lambda[
                "lambda_electrostatics_{}".format(region)
...
def lambda_trace(reference_system, alchemical_regions, positions, nsteps=100):
    ...
    for i in range(nsteps + 1):
        ...
        logger.info(
            "{:12.9f} {:24.8f} kcal/mol".format(
                lambda_i[i], u_i[i] / GLOBAL_ENERGY_UNIT
            )
        )
...
class TestAbsoluteAlchemicalFactory(object):
    ...
    def define_systems(cls):
        ...
        cls.test_systems[
            "LennardJonesFluid with dispersion correction"
        ...
        cls.test_systems[
            "HostGuestExplicit with CutoffPeriodic"
    ...
    def test_split_force_groups(self):
        ...
        for test_name, (
            ...
            f.description = "Testing force splitting among groups of {}".format(
                test_name
            )
            yield f
    ...
    def test_platforms(self):
        ...
        for platform in platforms:
            ...
            for test_name, (
                ...
                f.description = "Test fully interacting energy of {} on {}".format(
                    test_name, platform.getName()
                )
                yield f
                f = partial(
                    check_noninteracting_energy_components,
                ...
                f.description = "Test non-interacting energy of {} on {}".format(
                    test_name, platform.getName()
                )
                yield f
    ...
    def test_overlap(self):
        ...
        for test_name, (
            ...
            f.description = "Testing reference/alchemical overlap for {}".format(
                test_name
            )
            yield f
...
class TestDispersionlessAlchemicalFactory(object):
    ...
    def test_overlap(self):
        ...
        for test_name, (
            ...
            f.description = "Testing reference/alchemical overlap for no alchemical dispersion {}".format(
                test_name
            )
            yield f
...
class TestAbsoluteAlchemicalFactorySlow(TestAbsoluteAlchemicalFactory):
    ...
    def define_systems(cls):
        ...
        cls.test_systems = dict()
        cls.test_systems[
            "LennardJonesFluid without dispersion correction"
        ...
        cls.test_systems[
            "WaterBox with PME, switch, dispersion correction"
        ] = testsystems.WaterBox(
        ...
        for state, defined_lambdas in self.test_cases:
            ...
            for parameter in AlchemicalState._get_controlled_parameters():
                ...
                if parameter in defined_lambdas:
                    assert property_value == 1.0, "{}: {}".format(
                        parameter, property_value
                    )
                else:
                    assert property_value is None, "{}: {}".format(
                        parameter, property_value
                    )
