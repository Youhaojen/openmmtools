...
class TestHarmonicOscillatorsMultiStateSampler(object):
    ...
    def setup_class(cls):
        ...
        for state_index in range(cls.N_STATES + 2):
            sigma = (
                1.0 + 0.2 * state_index
            ) * unit.angstroms  # compute reasonable standard deviation with good overlap
        ...
        with self.temporary_storage_path() as storage_path:
            ...
            for key, value in options_to_store.items():
                if np.isscalar(value):
                    ...
                    ), "stored_options['%s'] = %s, but value = %s" % (
                        key,
                        stored_options[key],
                        value,
                    )
                    ...
                    ), "getattr(sampler, '%s') = %s, but value = %s" % (
                        "_" + key,
                        getattr(sampler, "_" + key),
                        value,
                    )
                else:
                    assert np.all(
    ...
    def test_minimize(self):
        ...
        with self.temporary_storage_path() as storage_path:
            ...
            for replica_index in node_replica_ids:
                ...
                ), "Energies did not decrease: Replica {} was originally {}, now {}".format(
                    replica_index, old_energy, new_energy
                )
